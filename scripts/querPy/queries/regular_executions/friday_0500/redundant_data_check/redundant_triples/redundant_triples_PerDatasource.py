...
title = ["redundant_triples_", datasources]


# description
# defines the textual and human-intended description of the purpose of these queries
# OPTIONAL, if not set, nothing will be used or displayed
description = "Queries which collect and count triples that exist multiple times over different graphs."
...
output_destination = r"https://drive.google.com/drive/folders/1fIJ3tFsUdzqicW75zu7WdDhsq4pJQaPI"
...
queries = [
    {
        "title": "most redundant triples, ordered by their number of associated graphs",
        "query": [r"""
            select ?s ?p ?o count ( ?source_graph ) as ?graph_count where {
                graph ?source_graph {
                    ?s ?p ?o
                }
            ...
            group by ?s ?p ?o order by desc(?graph_count)
        """]
    },
    {
        "title": "from redundant triples, get the count of their associated graphs (=graph_count), and then count how often this number occurs (=graph_count_count).",
        "query": [r"""
            select ?graph_count count(?graph_count) as ?graph_count_count where {
            ...
            group by ?graph_count order by desc(?graph_count)
        """]
    },
]
